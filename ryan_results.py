...
def getSystemDiskSpace(msg):
    ...
    msg += "used space:                 %.2f MBytes =\t%.2f GBytes\n" % (
        totalUsedSpace / 1024 / 1024,
        totalUsedSpace / 1024 / 1024 / 1024,
    )
    totalAvailSpace = float(disk.f_bsize * disk.f_bfree)
    msg += "available space:            %.2f MBytes =\t%.2f GBytes\n" % (
        totalAvailSpace / 1024 / 1024,
    ...
    msg += "available space (non-sudo): %.2f MBytes =\t%.2f GBytes\n" % (
        totalAvailSpaceNonRoot / 1024 / 1024,
...
def getRyanInfo(msg, logfile="info", n=10):
    ...
    for line in sh.tail(
        "-n",
        n,
        "/home/ubuntu/crawlers/scripts/viagens/ryanair/log/log_files/ryanair_%s.log"
        % logfile,
    ):
        txt += line
    msg += txt
...
def getFlightDataStats(f, msg, print_df=False):
    ...
    try:
        ...
        ddf = (
            df.fillna(0)
            .groupby(["dateTakeOff", "seatsRequired"])
            .agg({"flightNumber": pd.Series.nunique, "priceRegular": np.count_nonzero})
        )
        if print_df:
            msg += str(ddf) + "\n"
...
def prepareMsg(print_df=False, log_info=True):
    ...
    for fil in files:
        ...
        if f:
            ...
            msg += (
                "> File last uptadet:  %s" % time.ctime(os.path.getmtime(newest_file))
                + "\n"
            )
            msg += (
                "> File size:          %.2f Mb"
                % (os.path.getsize(newest_file) / 1024.0 / 1024.0)
                + "\n"
            )
            msg += "-" * 70 + "\n"
            msg += "Last files:" + "\n"
            if f:
                for ff in f[:5]:
                    msg += (
                        "-> %.2f Mb" % (os.path.getsize(ff) / 1024.0 / 1024.0)
                        + ff.split("/")[-1]
                        + "\n"
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-e", "--email", help="Enviar email com o log", action="store_true"
    )
    parser.add_argument(
        "-s", "--sleep", help="Tempo em segundos de espera", default=120
    )
    parser.add_argument(
        "-r", "--repeat", help="Repetir a cada [sleep] segundos", action="store_true"
    )
