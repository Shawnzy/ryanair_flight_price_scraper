...
def _db_connect():
    ...
    """
    return sqlite3.connect('/home/ubuntu/crawlers/scripts/viagens/ryanair/db/proxies.db')
...
def create_db():
    ...
    """
    resp = raw_input(u'>>> Voce tem certeza que pretende dropar a tabela "proxies"? y/[n]\n')
    if resp == 'y':
    ...
    with Lock():
        ...
        update_proxy = """
            ...
            SET timestamp = CURRENT_TIMESTAMP,
                ...
                useable = {useable}
            WHERE proxy = '{proxy}';
        """.format(proxy=proxy, mean_response_time=mean_response_time,
                    useable=useable)
        print(update_proxy)
        cursor = conn.cursor()
...
def _get_url():
    ...
    availability_url = 'https://desktopapps.ryanair.com/v4/en-ie/'+\
                        'availability?ADT='+random.choice(['1','2','3'])+\
                        '&CHD='+random.choice(['1','2','3'])+\
                        '&DateOut='+(datetime.today()+relativedelta.relativedelta(days=5)).strftime('%Y-%m-%d')+\
                        '&Destination='+random.choice(['BUD', 'DUB'])+\
                        '&FlexDaysOut=6'+\
                        '&INF=0&Origin='+random.choice(['FRA', 'STN'])+\
                        '&RoundTrip=false&TEEN='+random.choice(['1','2','3'])+\
                        '&ToUs=AGREED'
...
def _get_headers(cookie=''):
    ...
    headers = {'Host': 'desktopapps.ryanair.com',
...
def _request_flight_availability(url, proxy, headers,
                                 max_retries=3, request_timeout=45,
                                 sleep_time=1):
    """
    ...
    for retries_count in xrange(max_retries):
        ...
        msg = """
            ...
            > proxies: %s
            > cookies: %s
            """ % (url, str(headers), str(proxies), headers['Cookie'])
        print(msg)
        # Tenta fazer a requisição
        try:
            response = req.get(url, headers=headers, proxies=proxies, timeout=request_timeout)
            end_time = time()
            #print('*** Response:\n%s' % response.content)
            parsed_response = json.loads(response.content)
            if parsed_response.get('message', '').startswith('No HTTP resource was found that'):
                break
...
def _get_proxy(useable=True):
    ...
    with Lock():
        ...
        query_proxy = """
            ...
            ORDER BY timestamp
            LIMIT 1
        """.format(not_useable='' if useable else 'NOT')
        df = pd.read_sql(query_proxy, conn)
        if df.shape[0] > 0:
